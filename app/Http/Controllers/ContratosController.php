...
class ContratosController extends Controller
{
    ...
    public function state_oltcatv($id){
        ...
        if($contrato->state_olt_catv == true){
            ...
            CURLOPT_URL => 'https://interycom.smartolt.com/api/onu/enable_catv/'.$contrato->olt_sn_mac,
    ...
    public function exportar(Request $request){
        ...
        foreach ($contratos as $contrato) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue($letras[25].$i, isset($plan->nombre) ? $plan->precio : '')
                ...
                ->setCellValue($letras[28].$i, isset($servicio_otro->nombre) ? $servicio_otro->precio: '' )
                ;
            $i++;
        }
        ...
        ->setCellValue($letras[26].$i, number_format($totalServicioOtro, 0, ',', '.'))
        ;
    ...
    //funcion para iniciar contratos 
    public function contratoIniciar(Request $request ){
        $contrato = Contrato::find($request->id);
        if ($contrato) {
            $contrato->fecha_inicio_contrato = now(); // Establecer la fecha actual
        ...
        $contrato = Contrato::join('contactos as c', 'c.id', '=', 'contracts.client_id')->select('contracts.*', 'contracts.status as cs_status', 'c.nombre', 'c.apellido1', 'c.apellido2', 'c.nit', 'c.celular', 'c.telefono1', 'c.direccion', 'c.barrio', 'c.email', 'c.id as id_cliente', 'contracts.marca_router', 'contracts.modelo_router', 'contracts.marca_antena', 'contracts.modelo_antena', 'contracts.ip', 'contracts.grupo_corte', 'contracts.adjunto_a', 'contracts.referencia_a', 'contracts.adjunto_b', 'contracts.referencia_b', 'contracts.adjunto_c', 'contracts.referencia_c', 'contracts.adjunto_d', 'contracts.referencia_d', 'contracts.simple_queue', 'contracts.latitude', 'contracts.longitude', 'contracts.servicio_tv', 'contracts.contrato_permanencia', 'contracts.contrato_permanencia_meses', 'contracts.serial_onu', 'contracts.descuento', 'contracts.vendedor', 'contracts.canal', 'contracts.address_street', 'contracts.tecnologia', 'contracts.costo_reconexion', 'contracts.tipo_contrato', 'contracts.observaciones','contracts.fecha_inicio_contrato')->where('contracts.id', $request->id)->first();
