...
@endif
    ...
    <a href="{{route('ingresos.imprimir.nombre',['id' => $id, 'name'=> 'Ingreso No. '.$nro.'.pdf'])}}" target="_blanck"  class="btn btn-outline-primary @if(Auth::user()->rol==47) btn-xl @else btn-icons @endif" title="Imprimir"><i class="fas fa-print"></i></a>
@if($tipo!=3)
    @if($tipo!=4)
        <form action="{{ route('ingresos.anular',$id) }}" method="post" class="delete_form" style="display: none;" id="anular-ingreso{{$id}}">
            {{ csrf_field() }}
        </form>
