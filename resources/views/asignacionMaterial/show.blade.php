...
@section('content')
    ...
    <div class="paper">
        ...
        <hr>
        <form method="POST" action="" style="padding: 2% 3%;    " role="form" class="forms-sample" novalidate>
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                <select class="form-control selectpicker" name="id_tecnico" id="id_tecnico" required="" title="Seleccione" data-live-search="true" data-size="5" >
                                    <option value="{{$material_asignado->tecnico->id}}" selected disabled >{{$material_asignado->tecnico->nombres}}</option>
                                </select>
                            </div>
                        </div>
                    </div>
            ...
            <div class="fact-table">
                    ...
                    <div class="col-md-12">
                        <table class="table table-striped table-sm" id="table-form" width="100%">
                            ...
                            @foreach($material_asignado->items as $key => $item)
                                @php
                                    $material = \App\Model\Inventario\Inventario::find($item->id);
                                @endphp
                                <tr id="{{ $key + 1 }}">
                                    <td  class="no-padding">
                                        <select class="form-control selectpicker items_inv"  title="Seleccione" data-live-search="true" data-size="5"  disabled>
                                            <option value="{{$material->id}}" selected>{{$material->producto}} - ({{$material->ref}})</option>
                                        </select>
                                    </td>
                                    <td>
                                        <div class="resp-refer">
                                            <input type="text" class="form-control form-control"  value="{{ $material->ref }}" disabled>
                                        </div>
                                    </td>
                                    <!--
                                    ...
                                    <td>
                                        <input type="number" class="form-control"  value="{{ $item->cantidad }}" disabled>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                ...
                <div class="row" style="margin-top: 5%; padding: 3%; min-height: 180px;">
                    <div class="col-md-12 form-group">
                        ...
                        <textarea  class="form-control form-control-sm min_max_100" name="notas" disabled></textarea>
                    </div>
                </div>
